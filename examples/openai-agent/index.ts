...
const requestSchema = z.object({
  ...
  // Caller-owned ids only. openaiAgentsContext reads them; it never mints one.
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    // onto run({ context: { sessionId } }). Never randomUUID() here.
...
server.listen(port, "0.0.0.0", () => {
  console.log(`OpenAI Agents example listening on http://localhost:${port}`);
});
