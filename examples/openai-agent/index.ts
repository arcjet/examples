...
const requestSchema = z.object({
  ...
  // Caller-owned ids only. Copied onto run(..., { context: { sessionId } }).
  // openaiAgentsContext reads them; it never mints one.
...
function asPrintableId(value: string | undefined): string | undefined {
  ...
  // Same 1–256 printable-ASCII window openaiAgentsContext accepts.
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    // only copies that value onto run(..., { context: { sessionId } }).
    // Never randomUUID() per request here. Never call createAgentContext.
    // Never call session.getSessionId() — MemorySession mints a UUID.
    const sessionId = asPrintableId(input.conversationId);
    const ctx = openaiAgentsContext(
...
server.listen(port, "0.0.0.0", () => {
  console.log(`OpenAI agent example listening on http://localhost:${port}`);
});
