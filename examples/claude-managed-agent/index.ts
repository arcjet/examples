...
import { z } from "zod";
import { hasAnthropicKey, runAgent } from "./lib/agent.ts";
...
const requestSchema = z.object({
  ...
  // Caller-owned id only. Copied onto claudeManagedAgentsContext
  // ({ correlationId }). Never minted. Never an Anthropic session id.
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    const ctx = claudeManagedAgentsContext(
      conversationId === undefined ? undefined : { correlationId: conversationId },
    );
