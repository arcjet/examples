...
async function sendDenied(send, result) {
	try {
		...
		if (shouldWarn()) console.warn("@arcjet/guard: failed to send user.custom_tool_result after deny; returning the payload so the caller can retry:", error);
	}
	return {
		allowed: false,
		result
...
function errorResultFromUnknown(event, message) {
	...
	if (typeof record.session_thread_id === "string" && record.session_thread_id.length > 0) synthetic.session_thread_id = record.session_thread_id;
...
async function runHostedCustomTool(client, call, policy) {
	...
	if (!isCustomToolUseEvent(event)) {
		...
		if (fallback !== void 0) return sendDenied(send, fallback);
