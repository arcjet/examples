...
export type ManagedAgentsEventParams = UserMessageEventParams | UserCustomToolResultEventParams | {
  type: string;
};
 ...
 */
export interface ManagedAgentsRunnableTool<TInput = {
  [key: string]: unknown;
}, TOutput = unknown> {
  name?: string;
  run: (input: TInput, context?: unknown) => TOutput | Promise<TOutput>;
}
export declare function isUserMessageEvent(event: ManagedAgentsEventParams): event is UserMessageEventParams;
export declare function isCustomToolUseEvent(value: unknown): value is AgentCustomToolUseEvent;
/** Concatenate `user.message` text blocks for inbound rules. */
export declare function inboundTextFromEvents(events: readonly ManagedAgentsEventParams[]): string;
//#endregion