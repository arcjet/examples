...
* `invalid-label`.
*/
const LABEL_REJECTED_CODE = "AJ1023";
/**
...
*/
function labelRejectedByService(decision) {
...
*/
var ArcjetInvalidLabelError = class extends Error {
	/** The label as written, so a caller can report or log it. */
	label;
	constructor(label, where, problem) {
		super(`${where}: guard label ${JSON.stringify(label)} is invalid (${problem}). No policy can match it — rename the tool or pass an explicit action.`);
		this.name = "ArcjetInvalidLabelError";
		this.label = label;
	}
};
function isLowerAsciiLetterOrDigit(ch) {
...
function labelProblem(label) {
	...
	if (encoder.encode(label).length > MAX_LABEL_BYTES) return `longer than ${MAX_LABEL_BYTES} bytes`;
	const runes = [...label];
	if (!isLowerAsciiLetterOrDigit(runes[0])) return "must start with a lowercase letter or digit";
	if (!isLowerAsciiLetterOrDigit(runes.at(-1))) return "must end with a lowercase letter or digit";
	for (const ch of runes) {
		if (isLowerAsciiLetterOrDigit(ch) || ch === "-" || ch === "." || ch === "_") continue;
...
* spelling.
*/
function assertValidAction(action, where) {
	const problem = labelProblem(action);
	if (problem !== void 0) throw new ArcjetInvalidLabelError(action, where, problem);
}
/**
...
* ```
*/
function validateGuardLabel(label) {
	assertValidAction(label, "validateGuardLabel");
}
//#endregion
export { ArcjetInvalidLabelError, LABEL_REJECTED_CODE, assertValidAction, labelProblem, labelRejectedByService, validateGuardLabel };
