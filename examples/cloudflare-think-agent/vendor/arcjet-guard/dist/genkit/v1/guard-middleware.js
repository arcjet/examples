...
async function lookupRegisteredTool(ai, name) {
	...
	for (const key of candidates) try {
		...
		if (shouldWarn()) console.warn("@arcjet/guard: guardMiddleware() could not look up \"%s\"; guarding it as if it were unwrapped:", name, error);
	}
}
let middlewareSeq = 0;
/**
...
function guardMiddleware(client, policy = {}) {
	if (typeof policy.action === "string" && policy.action !== "") assertValidAction(policy.action, "guardMiddleware");
	return {
		name: middlewareName(),
		instantiate: (options) => {
			...
			return { tool: async (req, ctx, next) => {
				...
				const source = isContextSource(hookCtx) ? hookCtx : void 0;
