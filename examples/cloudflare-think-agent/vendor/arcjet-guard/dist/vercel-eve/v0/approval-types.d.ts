import { Approval, ApprovalContext, ApprovalStatus } from "eve/tools/approval";
//#region src/vercel-eve/v0/approval-types.d.ts
/**
 ...
 */
export type ApprovalPolicy<TInput = Record<string, unknown>> = Extract<Approval<TInput>, (ctx: ApprovalContext<TInput>) => unknown>;
export type ApprovalConfiguration<TInput = Record<string, unknown>> = Exclude<Approval<TInput>, ApprovalPolicy<TInput>>;
export type ApprovalResponsePolicy<TInput = Record<string, unknown>> = NonNullable<ApprovalConfiguration<TInput>["response"]>;
export type ApprovalResponseContext<TInput = Record<string, unknown>> = Parameters<ApprovalResponsePolicy<TInput>>[0];
