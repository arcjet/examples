...
async function gateToolCall(client, policy, params) {
	if (isBrandedTool(params.tool)) return;
...
*/
function isPluginHookName(prop) {
	return typeof prop === "string" && /^(?:before|after|on)[A-Z]\w*(?:Callback|Selection|Compaction)$/.test(prop);
}
function withUnknownHookNoops(plugin) {
	return new Proxy(plugin, { get(target, prop, _receiver) {
		const value = target[prop];
		if (value !== void 0) return value;
		if (isPluginHookName(prop)) return noopUndefined;
		return value;
	} });
}
/**
...
function guardPlugin(client, policy = {}) {
	if (typeof policy.action === "string" && policy.action !== "") assertValidAction(policy.action, "guardPlugin");
