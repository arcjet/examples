...
function policyForTool(tool, policy) {
	...
	return {
		...
		...actor !== void 0 && { actor: (input, config) => {
			const call = {
				toolName: tool.name,
				input
			};
		...
		...inputs !== void 0 && { inputs: (input, config) => {
			const call = {
				toolName: tool.name,
				input
			};
...
function guardToolNode(client, toolsOrNode, policy = {}) {
	if (typeof policy.action === "string" && policy.action !== "") assertValidAction(policy.action, "guardToolNode");
