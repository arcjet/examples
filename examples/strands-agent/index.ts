...
const requestSchema = z.object({
  ...
  // Caller-owned ids only. strandsAgentContext reads them; it never mints one.
...
function asPrintableId(value: string | undefined): string | undefined {
  ...
  // Same 1–256 printable-ASCII window strandsAgentContext accepts.
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    // onto invoke({ invocationState: { sessionId } }). Never randomUUID() here.
    const sessionId = asPrintableId(input.conversationId);
    const ctx = strandsAgentContext(
      sessionId === undefined ? undefined : { invocationState: { sessionId } },
    );
