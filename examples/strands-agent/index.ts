...
const requestSchema = z.object({
  ...
  // Caller-owned id only. Copied onto invocationState.sessionId. Never minted.
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    sendJson(response, 200, {
      ...
      hookDenials: generated.hookDenials,
...
server.listen(port, "0.0.0.0", () => {
  console.log(`Strands agent example listening on http://localhost:${port}`);
});
