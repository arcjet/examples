...
import { GuardRuleSubmissionSchema, GuardRuleSchema, RuleTokenBucketSchema, RuleFixedWindowSchema, RuleSlidingWindowSchema, RuleDetectPromptInjectionSchema, RuleModerateContentSchema, RuleLocalSensitiveInfoSchema, RuleLocalCustomSchema, ResultLocalSensitiveInfoSchema, GuardSensitiveInfoEntitySchema, ResultLocalCustomSchema, ResultErrorSchema, EntityListSchema, GuardConclusion, GuardPolicyStatus, GuardReason, GuardRuleMode, GuardRuleExecution, GuardStringMatchOperator, } from "./proto/proto/decide/v2/decide_pb.js";
...
const analyzeContext = { log: noopLog, characteristics: [] };
/**
 ...
 * @internal
 */
export const nativeEntityTypes = new Set([
    "EMAIL",
    "PHONE_NUMBER",
    "IP_ADDRESS",
    "CREDIT_CARD_NUMBER",
...
export function isSensitiveInfoEntityType(value) {
 ...
 */
const wasmSensitiveInfoBackend = {
    detect(context, value, entities, options) {
        ...
        { log: context.log, characteristics: [] }, value, entities, options?.contextWindowSize ?? 1, options?.detect);
    },
};
/**
 ...
 * @internal
 */
export function conclusionFromProto(c) {
    switch (c) {
 ...
 * @internal
 */
export function reasonFromCase(caseName) {
    switch (caseName) {
        case "tokenBucket":
        case "fixedWindow":
        case "slidingWindow":
 ...
 * @internal
 */
export function reasonFromProto(r) {
    switch (r) {
 ...
 * @internal
 */
export function resultFromProto(pr) {
...
function policyResultFromProto(pr) {
    ...
    switch (pr.result.case) {
        ...
        case "stringLength":
            result = {
                ...
                type: pr.result.case === "allowedStringValues"
                ...
                ...(pr.result.case === "stringLength"
                    ? {}
                    : {
                        matchOperator: pr.result.value.matchOperator === GuardStringMatchOperator.EMAIL_DOMAIN
                            ? "EMAIL_DOMAIN"
    ...
    return {
        ...
        execution: pr.execution === GuardRuleExecution.SDK
            ? "SDK"
 ...
 */
export async function ruleToProto(rule, signal, options) {
    ...
    switch (rule.type) {
        ...
        case "SENSITIVE_INFO": {
            ...
            const entities = rule.config.deny
                ? { tag: "deny", val: rule.config.deny.map((s) => stringToEntity(s)) }
                : { tag: "allow", val: (rule.config.allow ?? []).map((s) => stringToEntity(s)) };
            let localResult;
        ...
        case "CUSTOM": {
            ...
            if (rule.evaluate) {
                const evalStart = performance.now();
                try {
                    const evalResult = await rule.evaluate(rule.config.data ?? {}, rule.input.data, signal === undefined ? {} : { signal });
                    resultDurationMs = BigInt(Math.round(performance.now() - evalStart));
 ...
 * @internal
 */
export function decisionMembers(conclusion, results, warnings, additionalErrors = []) {
    // Compute once; both accessors close over the same array.
    const errored = [
        ...results.filter((r) => r.type === "RULE_ERROR"),
        ...additionalErrors,
    ];
 ...
 */
export function decisionFromProto(response, _rules, localWarnings = []) {
