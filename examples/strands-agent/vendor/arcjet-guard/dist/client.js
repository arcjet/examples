...
import { ruleToProto, decisionFromProto, decisionMembers } from "./convert.js";
...
import { DecideService, CaptureEventSchema, CaptureRequestSchema, GuardRequestSchema, GuardConclusion, GuardDecisionSchema, GuardPolicyEvaluationSchema, GuardPolicyRuleResultSchema, GuardPolicyStatus, GuardReason, GuardRuleExecution, GuardRuleMode, GuardRuleSource, GuardResponseSchema, WarningSchema, } from "./proto/proto/decide/v2/decide_pb.js";
import { policyCapabilities, RemotePolicyRuntime } from "./remote-policy.js";
import { symbolArcjetInternal } from "./symbol.js";
import { userAgent as defaultUserAgent } from "./version.js";
/**
 ...
 */
export function createGuardClient(options) {
    const { key, transport, userAgent = defaultUserAgent() } = options;
    const client = createConnectClient(DecideService, transport);
    const remotePolicy = new RemotePolicyRuntime(key, userAgent, (request, callOptions) => client.getGuardPolicy(request, callOptions), options.sensitiveInfoBackend);
    // Spread rather than `{ logger: options.logger }`: under
    // `exactOptionalPropertyTypes` an explicit `undefined` is not assignable to an
    // optional property.
    const diagnose = createDiagnosticHandler(options.logger === undefined ? {} : { logger: options.logger });
    ...
    return {
        ...
        async guard(opts) {
            ...
            const timeoutMs = opts.timeoutSeconds !== undefined && opts.timeoutSeconds !== 0
            ...
            warnings.push(...requestMetadata.localWarnings, ...enforceMetadataBudget([
            ...
            try {
                ...
                const shouldRefresh = opts.inputs !== undefined &&
                ...
                opts.signal?.throwIfAborted();
                const message = cause instanceof ConnectError
 ...
 */
export function normalizeCaptureEvent(value, diagnose) {
    try {
        ...
        const occurredAtUnixMs = normalized.occurredAt === undefined
            ? BigInt(Date.now())
 ...
 * `exports` map lists no path that resolves here.
 */
export function createFailOpenDecision(message, warnings = []) {
...
function localPolicyDenial(preparedPolicy, warnings) {
    ...
    return decisionFromProto(create(GuardResponseSchema, {
