...
# Arcjet example: Strands agent guardrails

[Arcjet](https://arcjet.com) helps developers protect their apps in just a few
lines of code. Bot detection. Rate limiting. Email validation. Attack
protection. Data redaction. A developer-first approach to security.

This is an example [Strands Agents JS](https://strandsagents.com/)
(`@strands-agents/sdk` `Agent` + `tool({ callback })` + Plugin / `addHook`)
agent protected by
[Arcjet AI guardrails](https://docs.arcjet.com/guards/strands-agents/). A
support agent looks up orders and notifies a warehouse. Arcjet screens inbound
...
correlated from the caller-owned conversation id passed as
`invocationState.sessionId` — the example never mints a new one.

This is the JS SDK, not the Python SDK. Docs slug:
...
> [!IMPORTANT]
> This example depends on the Arcjet Guard **Strands Agents** integration
...
## Screen inbound before `agent.invoke()` / `stream()`

There is no first-class Strands inbound channel, so there is no
`guardInbound`. Put `detectPromptInjection` in the application before
`agent.invoke()` / `stream()`. Middleware / model hooks intercept the
...
`event.interrupt()` is human-in-the-loop, not policy. Same trap as Mastra
`requireApproval`, Claude `canUseTool`, LangGraph `interrupt()`, OpenAI
Agents `needsApproval`, and LangChain `humanInTheLoopMiddleware`. There is
no `guardApproval` / `guardInterrupt`. Do not wrap `interrupt()` as Guard.

This example leaves HITL as a comment in `lib/agent.ts`. That pause is not a
deny — Guard still evaluates when `guardTool` / `guardHooks` run.

## Deny with `BeforeToolCallEvent.cancel` (and `guardTool` on authored callbacks)

Do not collapse these. Do not throw on DENY (throws abort the invocation
and drop `arcjetDenied`). Do not use `BeforeToolsEvent.cancel` — a truthy
value skips per-tool hooks.

- **`guardTool`** returns a plain `ArcjetDenialResult`
  (`{ arcjetDenied: true, reason, message, retryable }`) from the authored
  `callback`. It does not fabricate a `ToolResultBlock`. `FunctionTool`
  wraps that object in a `JsonBlock`.
- **`guardHooks` `BeforeToolCallEvent`** sets `event.cancel` to
...
- An authored tool (`lookup_order`) wrapped with `guardTool` uses a
  ...
  `arcjetDenied: true` — the wrapper does not throw.
...
- An unwrapped tool (`notify_warehouse`) is gated with `guardHooks`
  (`BeforeToolCallEvent.cancel`). Do not also wrap that tool with
  `guardTool`, `@arcjet/guard/langgraph/v1`, or
...
- Every helper uses `onGuardError: "deny"` (fail closed). If Arcjet is
  ...
  denial.
- Correlation is read by `strandsAgentContext` from
  `invocationState.sessionId` / `correlationId`. The server never calls
  `createAgentContext`, never mints an id, and never reads `traceId`,
  `agent.id`, or `SessionManager`.
...
6. Try the example prompts:
   ...
   - **HITL note:** `interrupt()` is a pause, not a deny. This example does
     not call it. Guard still runs in `guardTool` / `guardHooks`.
...
- `AI_GATEWAY_API_KEY` — used by Strands Agents to call the model that
...
- **Authored tool:** `guardTool` on `lookup_order` — rate limit and PII on
  the `note` argument. The model receives a `JsonBlock` whose content is
  `{ arcjetDenied, reason, message, retryable }`. Explain the denial instead
  of retrying.
- **Unwrapped tool:** `guardHooks` `BeforeToolCallEvent.cancel` on
  `notify_warehouse`. DENY is a cancel string whose JSON is the same
...
`strandsAgentContext` reads a field the integrator put on
`invocationState`. It never mints a new id:

1. **`invocationState.correlationId`** — prefer this when you already
   have a correlation id.
2. **`invocationState.sessionId`** — the conversation id from the
   request. This example copies the caller-owned id here so every turn
   in a conversation joins one Sequence.
3. **`invocationState.requestId`**, then documented copies on the
   envelope, then `init.sessionId` / `init.correlationId`.
...
`createAgentContext` inside a Strands callback — that would mint a second
id and split the Sequence. Do not read `traceId`. Do not use
`SessionManager` or `agent.id`. Do not treat `interrupt` / resume as
correlation.
...
`invocationState.sessionId`. It never calls `randomUUID()` per request.
